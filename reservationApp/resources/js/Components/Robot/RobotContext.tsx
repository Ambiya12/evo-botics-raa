...
import { useRosBridge, type RosLogEntry, type RosStatus } from '@/hooks/useRosBridge';
import { useRobotTelemetry, type RobotTelemetry } from '@/hooks/useRobotTelemetry';
...
type RobotContextValue = {
    config: RobotConfig;
    setConfig: <K extends keyof RobotConfig>(key: K, value: RobotConfig[K]) => void;
    rosUrl: string;
    cameraUrl: string;
    status: RosStatus;
...
export function RobotProvider({ children }: { children: ReactNode }) {
    ...
    const setConfig = useCallback(<K extends keyof RobotConfig,>(key: K, value: RobotConfig[K]) => {
        setConfigState((current) => ({ ...current, [key]: value }));
    }, []);

    const rosUrl = useMemo(() => `ws://${config.robotHost}:${config.rosPort}`, [config.robotHost, config.rosPort]);
    const cameraUrl = useMemo(() => {
        const normalizedPath = config.cameraPath.startsWith('/') ? config.cameraPath : `/${config.cameraPath}`;
    ...
    const sendGoal = useCallback((x: number, y: number, yaw = 0) => {
        ros.publish('/goal_pose', 'geometry_msgs/msg/PoseStamped', {
            header: { frame_id: 'map' },
            pose: { position: { x, y, z: 0 }, orientation: quaternionFromYaw(yaw) },
        });
    ...
    const value = useMemo<RobotContextValue>(() => ({
        ...
        sendGoal,
        saveMap,
    }), [config, setConfig, rosUrl, cameraUrl, rosBridge.status, rosBridge.lastMessageAt, rosBridge.logs, rosBridge.clearLogs, ros, telemetry, waypoints, addCurrentPoseWaypoint, sendGoal, saveMap]);
