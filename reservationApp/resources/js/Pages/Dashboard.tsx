...
export default function Dashboard() {
    ...
    const { reservations } = usePage<{ reservations: Reservation[] }>().props;
                                ...
                                <div className="p-6 text-gray-900">
                                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className={`flex h-12 w-12 items-center justify-center rounded-full ${item.status === 'confirmed' || item.status === 'validated' ? 'bg-green-100' :
                                                item.status === 'pending' ? 'bg-yellow-100' :
                                                    item.status === 'cancelled' ? 'bg-red-100' : 'bg-gray-100'
                                                }`}>
                                                <svg className={`h-6 w-6 ${item.status === 'confirmed' || item.status === 'validated' ? 'text-green-600' :
                                            ...
                                            <div>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${item.status === 'confirmed' || item.status === 'validated' ? 'bg-green-110 text-green-700' :
                                                        item.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                                                            item.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'
                                                        }`}>
