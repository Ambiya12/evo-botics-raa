    ...
    <main>
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-3"
                 x-data="{
                    ...
                    async book() {
                        ...
                        await $wire.reserve();
                        setTimeout(() => $wire.checkAvailability(), 3000);
                    },
                    ...
                    get monthName() {
                        return new Date(this.year, this.month).toLocaleString('en', { month: 'long' });
                    },
                    get formattedDate() {
                    ...
                    async selectDate(day) {
                        ...
                        this.localStartTime = '';
                        this.localEndTime = '';
                        await $wire.set('selectedDate', d);
                        await $wire.set('selectedStartTime', '');
                        await $wire.set('selectedEndTime', '');
                        this.showTimePicker = true;
                    },
                    async selectStartTime(hour) {
                        const t = String(hour).padStart(2, '0') + ':00';
                        this.localStartTime = t;
                        await $wire.set('selectedStartTime', t);
                        const endHour = Math.min(hour + 1, 20);
                        const endT = String(endHour).padStart(2, '0') + ':00';
                        this.localEndTime = endT;
                        await $wire.set('selectedEndTime', endT);
                    },
                    async selectEndTime(hour) {
                        const t = String(hour).padStart(2, '0') + ':00';
                        this.localEndTime = t;
                        await $wire.set('selectedEndTime', t);
                        this.showTimePicker = false;
                    },
                    async syncName() {
                        await $wire.set('name', this.localName);
                    },
                    async syncEmail() {
                        await $wire.set('email', this.localEmail);
                    },
                    async updatePeople(val) {
                        this.localPeople = val;
                        await $wire.set('peopleCount', val);
                    }
                 }">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div x-show="showTimePicker"
                        ...
                        <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl">
                            <h2 class="mb-4 text-xl font-bold">Choose your time slot</h2>
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    <p class="mb-2 text-sm font-semibold text-gray-500">Start</p>
                                    <div class="space-y-1">
                                ...
                                <div>
                                    <p class="mb-2 text-sm font-semibold text-gray-500">End</p>
                                    <div class="space-y-1">
                                        <template x-if="!localStartTime">
                                            <p class="text-sm text-gray-400">Pick start time first</p>
                                        </template>
            ...
            <div class="lg:col-span-3 flex flex-col items-center justify-center py-32">
                ...
                <h2 class="mt-6 text-2xl font-semibold text-gray-900">Checking availability...</h2>
                <p class="mt-2 text-gray-500">We are looking for an available room, please wait.</p>
            </div>
            ...
            <div class="lg:col-span-3 flex flex-col items-center justify-center py-32">
                ...
                <h2 class="mt-6 text-2xl font-semibold {{ $roomFound ? 'text-green-700' : 'text-red-700' }}">
                    {{ $roomFound ? 'Room found!' : 'No room available' }}
                </h2>
