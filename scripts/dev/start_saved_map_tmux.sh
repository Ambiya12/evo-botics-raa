...
SESSION="${SESSION:-evo-saved-map}"
ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-30}"
FASTDDS_BUILTIN_TRANSPORTS="${FASTDDS_BUILTIN_TRANSPORTS:-UDPv4}"
MAP_PATH="${MAP_PATH:-/root/maps/admin_map.yaml}"
...
remote_env=(
  ...
  "MAP_PATH=$(remote_quote "$MAP_PATH")"
...
echo "[saved-map-tmux] Preparing ${SESSION} on ${JETSON_TARGET}"

ssh "$JETSON_TARGET" "${remote_env[*]} bash -s" <<'REMOTE_SCRIPT'
set -euo pipefail

if ! command -v tmux >/dev/null 2>&1; then
  echo "[saved-map-tmux] tmux is not installed on the Jetson."
  echo "[saved-map-tmux] Install it once with: sudo apt-get install -y tmux"
  exit 1
fi

if ! command -v docker >/dev/null 2>&1; then
  echo "[saved-map-tmux] docker is not available on the Jetson."
  exit 1
fi

if ! docker ps --format '{{.Names}}' | grep -Fxq "$CONTAINER"; then
  echo "[saved-map-tmux] Docker container '${CONTAINER}' is not running."
  echo "[saved-map-tmux] Running containers:"
  docker ps --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}'
  exit 1
fi

if ! docker exec "$CONTAINER" test -f "$MAP_PATH"; then
  echo "[saved-map-tmux] Map file does not exist inside Docker: ${MAP_PATH}"
  echo "[saved-map-tmux] Override with: MAP_PATH=/root/maps/your_map.yaml ./scripts/dev/start_saved_map_tmux.sh"
  exit 1
fi

if tmux has-session -t "$SESSION" 2>/dev/null; then
  echo "[saved-map-tmux] Session '${SESSION}' already exists."
  exit 0
fi
...
tmux new-window -t "$SESSION:" -n nav \
  "$(docker_ros_cmd "ros2 launch evo_navigation navigation.launch.py map:=${MAP_PATH} rviz:=false")"
...
echo "[saved-map-tmux] Started '${SESSION}' with windows: bringup, camera, nav, web"
REMOTE_SCRIPT

if [[ "$ATTACH" == "1" ]]; then
  echo "[saved-map-tmux] Attaching. Detach with: Ctrl-b then d"
  ssh -t "$JETSON_TARGET" "tmux attach-session -t $(remote_quote "$SESSION")"
else
  echo "[saved-map-tmux] Attach later with:"
